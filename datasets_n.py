...
def make_cure_data(n_src_tr,n_src_val,n_trg_val,n_trg_test,natural_type="Haze",):
    Xs = []
    Ys = []
    Xs_src =[]
    Ys_src = []
    datagen = ImageDataGenerator(rescale=1./255)
    data_generator = datagen.flow_from_directory(
        "./CURE-TSR/Real_Train/ChallengeFree", shuffle=False, target_size=(28,28), **image_options)
    while True:
        next_x, next_y = data_generator.next()
    ...
    for i in range(5):
        data_generator = datagen.flow_from_directory(
        "./CURE-TSR/Real_Train/"+natural_type+"-"+str(i+1), shuffle=False, target_size=(28,28), **image_options)
        while True:
            next_x, next_y = data_generator.next()
            Xs.append(next_x)
            Ys.append(next_y)
    ...
    return (src_tr_x, src_tr_y, src_val_x, src_val_y, inter_x, inter_y,trg_val_x, trg_val_y, trg_test_x, trg_test_y)

def cure_data_func():
    return make_cure_data(1000, 5000,2000,5000,2000)